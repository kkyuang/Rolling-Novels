...
app.get('/', function(request, response) {
  ...
  novelList.sort((a, b) => b.paragraph[b.paragraph.length - 1].time - a.paragraph[a.paragraph.length - 1].time)
  for(var i = 0; i < novelList.length; i++){
    ...
    <span style="font-size: 0.6em;"><br><br>최초 생성: ${getTermText(novelList[i].time)}<br>최종 이어쓰기: ${getTermText(novelList[i].paragraph[novelList[i].paragraph.length - 1].time)}</span></button>`
  }
...
app.get('/chat', function(request, response) {
    var html = readHTML('chat') + `<script>var myname = '${request.session.logined ? request.session.user_id : '익명' + Math.floor(Math.random() * 10000)}'</script>`
    html = changeElements(html, [{'key': 'roomName', 'value': '광장'}])
    response.send(html)
});

//채팅 서비스 이용
app.get('/chat/private/:roomname', function(request, response) {
    var html = readHTML('chat') + `<script>var myname = '${request.session.logined ? request.session.user_id : '익명' + Math.floor(Math.random() * 10000)}'</script>`
    html = changeElements(html, [{'key': 'roomName', 'value': request.params.roomname}])
    response.send(html)
});
...
io.on('connection', (socket) => {
  ...
  socket.on('enter_room', (data)=>{
    ...
    //입장 메시지
    var msgData = {
      nickname: removeTag('시스템'),
      msg: removeTag(data.nickname + ' 님이 입장하셨습니다.'),
      id: chatCount(data.roomName)
    }


    //최근 채팅내용 전송
    now100 = Math.floor(chatCount(data.roomName) / 100)
    if(chatCount(data.roomName) % 100 == 0){
      now100-=1
    }
    ...
    socket.emit('chatList', {
      nowchat100: now100,
      chatList:chatRead(data.roomName, now100)
    })

    //io.to(data.roomName).emit('newchat', msgData)
    //chatSave(data.roomName, msgData)
  })
  ...
  socket.on('chatListReq', (data)=>{
    socket.emit('chatListRes', chatRead(data.roomName, data.num))
  })

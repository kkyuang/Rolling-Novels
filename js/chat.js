...
function sendChat(socket){
    ...
    socket.emit("chat", {msg: msg, nickname: nickname, 'roomName': roomName})
...
$(document).ready(() => {
    ...
    //방에 입장
    console.log('hello')
    socket.emit("enter_room", {nickname: nickname, roomName: roomName})
    ...
    $('#chat-scrolls').scroll(()=>{
        if(isScrollTop()){ //맨위면
            ...
            if(nowchat100 != 0){ //다 로드되지 않았다면
                socket.emit('chatListReq', {'num': nowchat100 - 1, 'roomName': roomName}) //요청보내기
                nowchat100 -= 1
            }
        }
    })
    ...
    socket.on('chatList', (data) => { //채팅 리스트 얻어오기
        ...
        for(var i = 0; i < chatList.length; i++){
            console.log(`${chatList[i].nickname}: ${chatList[i].msg} ` + i )
            $('#chat-contents').append(`
            <div id="one-chat" style="text-align: left;">
